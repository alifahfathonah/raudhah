...
class PaymentsImport implements ToModel, WithStartRow
{
	...
	*/
	public function model(array $row)
	{
		...
		// 	);
		// }
		$notverified = Registrant::where('nova', $nova)->where('isverified', false)->first();
		$dataexists	= Payment::where('paynumber', $nova)->first();
		if($notverified){
			Registrant::where('nova', $nova)->update(['paydate' => $date, 'isverified' => true]);
			if(!$dataexists){
				return new Payment([
						'paydate'    => $date,
						'paynumber'  => $nova, 
