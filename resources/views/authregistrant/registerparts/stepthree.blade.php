...
<form class="ui form basic segment @if ($errors->any()) error @endif" method="POST" action="{{route('register.step.3')}}">
	...
	<div class="field">
		...
		<div class="fields">
			...
			<div class="ten wide field required @error('schname') error @enderror">
				...
				<input type="text" name="schname" class="uppercase-input" value="{{old('schname', '')}}">
			</div>
		</div>
		
		<div class="field required @error('schstreet') error @enderror">
			<label>Alamat Sekolah</label>
			<input type="text" name="schstreet" class="uppercase-input" value="{{old('schstreet', '')}}">
		</div>		
		...
		<div class="two fields">
			<div class="field">
				...
				<input type="text" name="schpsn" class="numeric-input" value="{{old('schpsn', '')}}">
			</div>
			<div class="field">
				<label>No. Peserta Ujian Negara (UN)</label>
				<input type="text" name="schun" class="numeric-input" value="{{old('schun', '')}}">
			</div>
		</div>
		...
		<div class="two fields">
			<div class="field">
				...
				<input type="text" name="schijazah" class="numeric-input" value="{{old('schijazah', '')}}">
			</div>
			<div class="field">
				<label>Nomor SKHUN</label>
				<input type="text" name="schskhun" class="numeric-input" value="{{old('schskhun', '')}}">
			</div>
		</div>
	...
	<div class="field" id="form-pindahan">
		...
		<div class="field required @error('psnfrom') error @enderror">
			...
			<input type="text" name="psnfrom" class="uppercase-input" value="{{old('psnfrom', '')}}">
		</div>
		
		<div class="field required @error('psnadd') error @enderror">
			<label>Alamat Pesantren</label>
			<textarea name="psnadd" rows="2" class="uppercase-input">{{old('psnadd', '')}}</textarea>
		</div>
		...
		<div class="fields">
			...
			<div class="twelve wide field required @error('psndesc') error @enderror">
				...
				<input type="text" name="psndesc" class="uppercase-input" value="{{old('psndesc', '')}}">
			</div>
		</div>
		...
		<div class="four fields">
			<div class="field required @error('psnup') error @enderror">
				...
				<input type="text" name="psnup" class="numeric-input" value="{{old('psnup', '')}}">
			</div>
