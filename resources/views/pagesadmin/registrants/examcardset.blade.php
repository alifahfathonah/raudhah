	...
	<div class="col-12">
		<div class="card shadow mb-4">
			...
			<div class="card-header py-2 d-flex flex-row align-items-center justify-content-between">
				<h6 class="m-0 font-weight-bold text-custom">Kartu Ujian {{$reg->name}}</h6>
			...
			<form action="{{route('admin.examcard.store')}}" method="post">
				@csrf
				<input type="hidden" name="idtostore" value="{{$reg->id}}">
				<div class="card-body">
					<div class="row">
						<div class="col">
					...
					<div class="row">
						...
						<div class="form-group col-sm-12 col-md-6">
							...
							<input name="numchar" type="text" class="form-control @error('numchar') is-invalid @enderror" value="{{old('numchar')}}" autofocus>
						</div>
						{{-- foodtable --}}
						<div class="form-group col-sm-12 col-md-6">
							...
							<select name="foodtable" id="foodtable" class="form-control @error('foodtable') is-invalid @enderror">
								...
								@foreach ($tables as $t)
								<option value="{{$t->id}}"{{$t->vnow == $t->capacity ? ' disabled' : ''}} {{old('foodtable') == $t->id ? 'selected' : ''}}>{{$t->name}} - {{$t->capacity - $t->vnow}}/{{$t->capacity}}</option>
								@endforeach
							</select>
						</div>
						{{-- building class --}}
		...
		$.ajax({
			...
			success: function(res){
				...
				$.each(res.data, function(i, v){
					...
					$(selid).append('<option value="'+ v.id +'"'+ disabled +'>'+ v.name +' - ('+ sisa +'/'+ v.capacity +')</option>');
				});
			},
		});
	}
